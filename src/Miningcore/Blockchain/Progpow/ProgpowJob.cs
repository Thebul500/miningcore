...
#pragma warning restore 618
      ...
      StratumConnection worker, ulong nonce, string inputHeaderHash, string mixHash)
    {
        ...
        //logger.Info($"Submitted header-hash: {inputHeaderHash}");
        //logger.Info($"Computed header-hash: {headerHashHex}");
        ...
        var computedMixHashHex = mixHashOut.ToHexString();

        //logger.Info($"Submitted mix-hash: {mixHash}");
        //logger.Info($"Computed mix-hash: {computedMixHashHex}");

        if(mixHash != computedMixHashHex)
        ...
        // calc share-diff
        var shareDiff = (double) new BigRational(PhicoinConstants.Diff1, resultValueBig) * shareMultiplier;
        ...
        IHashAlgorithm headerHasher, IHashAlgorithm blockHasher, IProgpowCache progpowHasher)
    {
        ...
        this.extraNoncePlaceHolderLength = PhicoinConstants.ExtranoncePlaceHolderLength;
